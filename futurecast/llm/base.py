...
# Fields that agent layers may attach to assistant message dicts for logging/context
# but that Chat Completions APIs will reject.  Both the agent layer (which writes
# these fields) and any Chat Completions client (which must strip them) should
# reference this constant so that a rename can't silently break the roundtrip.
ASSISTANT_MESSAGE_EXTRA_FIELDS: frozenset = frozenset({"reasoning_summary"})
...
class LLMResponse:
    ...
    Attributes:
        ...
        reasoning_summary: Provider-returned summary of reasoning, when exposed
...
class OpenAISDKClient(BaseLLMClient):
    ...
    def _extract_reasoning_summary_texts(self, value: Any) -> List[str]:
        """Collect provider-exposed reasoning summaries without interpreting them."""
        if value is None:
            return []
        ...
        if isinstance(value, dict):
            ...
            if value_type == "reasoning.encrypted":
                logger.debug(
                    "Skipping encrypted reasoning token (provider-side redaction); "
                    "the model is thinking but the gateway is not exposing the summary."
                )
                return []
            for key in ("summary", "text"):
                texts.extend(self._extract_reasoning_summary_texts(value.get(key)))
            return texts
        ...
        texts = []
        for key in ("summary", "text"):
            texts.extend(self._extract_reasoning_summary_texts(getattr(value, key, None)))
        return texts

    def _extract_chat_completion_reasoning_summary(
        self,
        response: Any,
        message: Any,
    ) -> List[str]:
        ...
        texts.extend(
            self._extract_reasoning_summary_texts(
                self.get_field_value(message, "reasoning_details")
            )
        )
    ...
    def _parse_chat_completion_response(
        ...
        *,
        ...
        return LLMResponse(
            ...
            reasoning_summary=self._extract_chat_completion_reasoning_summary(
                response,
                message,
            ),
    ...
    async def stream_chat(self, messages, tools=None, **kwargs):
        """Single-chunk fallback for clients that don't support true streaming.

        Subclasses with real streaming capability (e.g. OpenRouterNewAPIClient)
