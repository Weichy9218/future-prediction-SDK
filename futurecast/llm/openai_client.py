...
class OpenAIClient(OpenAISDKClient):
    ...
    def _extract_responses_reasoning_summary(self, response: Any) -> List[str]:
        output = self.get_field_value(response, "output")
        texts: List[str] = []
        for item in output or []:
            if self.get_field_value(item, "type") != "reasoning":
                continue
            texts.extend(
                self._extract_reasoning_summary_texts(
                    self.get_field_value(item, "summary")
                )
            )
        return texts

    def _supports_temperature(self) -> bool:
        # gpt-5.x uses fixed sampling; sending temperature causes a 400.
        # This heuristic covers OpenAI-native model names only — OpenRouter
        # uses prefixed names (e.g. "openai/gpt-5.x") and manages temperature
        # itself in _build_request_params, so it never reaches this method.
        return not str(self.model or "").strip().lower().startswith("gpt-5")
    ...
    def _build_responses_request_params(
        ...
        **kwargs,
        ...
        if "reasoning" not in params:
            ...
            if effort:
                # "summary": "auto" asks OpenAI to return a human-readable reasoning
                # summary alongside the assistant output.  Without it the field is
                # omitted even when the model performs extended thinking.
    ...
    async def _do_responses_create(self, client: Any, params: Dict[str, Any]) -> Any:
        """Dispatch one Responses API call through the given SDK client (async or sync)."""
        if self.async_mode:
    ...
    async def _create_response(self, params: Dict[str, Any]):
        ...
        for _ in range(3):
            try:
                response = await self._do_responses_create(self.client, params)
                break
            except APITimeoutError as exc:
                logger.error("OpenAI API timeout: %s (model=%s)", exc, self.model)
                raise
    ...
    def _build_responses_llm_response(
        ...
        *,
        ...
        return LLMResponse(
            ...
            reasoning_summary=self._extract_responses_reasoning_summary(response),
            usage=usage,
