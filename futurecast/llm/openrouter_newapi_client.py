...
from .base import ASSISTANT_MESSAGE_EXTRA_FIELDS, LLMResponse, OpenAISDKClient, register_llm_client
...
def _is_forced_tool_choice_unsupported_error(error: object) -> bool:
    """Detect gateway routes that allow tools but reject forced tool_choice."""
    message = str(error).lower()
    return (
        "thinking mode does not support this tool_choice" in message
        or "does not support this tool_choice" in message
    )


def _can_retry_with_auto_tool_choice(params: Dict[str, Any]) -> bool:
    tool_choice = params.get("tool_choice")
    return bool(params.get("tools")) and isinstance(tool_choice, dict)


def _normalize_json_schema_for_openai_compatible(value: Any) -> Any:
    """Return a schema copy accepted by stricter OpenAI-compatible gateways."""
    if isinstance(value, list):
        return [_normalize_json_schema_for_openai_compatible(item) for item in value]
    ...
    for key, item in value.items():
        if key == "required" and item is None:
            ...
            normalized[key] = _normalize_json_schema_for_openai_compatible(item)
    if normalized.get("type") == "object" and "required" not in normalized:
        normalized["required"] = []
    return normalized


def _sanitize_tools_for_openai_compatible(
    tools: Optional[List[Dict[str, Any]]],
) -> Optional[List[Dict[str, Any]]]:
    """Normalize MCP-derived tool schemas without mutating the tool registry."""
    if not tools:
        return tools
    return [
        _normalize_json_schema_for_openai_compatible(copy.deepcopy(tool))
        for tool in tools
    ]
...
class OpenRouterNewAPIClient(OpenAISDKClient):
    ...
    def __init__(
        ...
        disable_forced_tool_choice: bool = False,
        ...
        **kwargs,
        ...
        self.disable_forced_tool_choice = disable_forced_tool_choice
    ...
    def _build_request_params(
        ...
        **kwargs,
        ...
        processed_tools = _sanitize_tools_for_openai_compatible(tools)
        ...
        if processed_tools:
            params["tools"] = processed_tools
            resolved_tool_choice = tool_choice or "auto"
            if self.disable_forced_tool_choice and isinstance(resolved_tool_choice, dict):
        ...
        while retry_count < _DEFAULT_MAX_RETRIES:
            try:
                ...
                if (
                    ...
                    and _can_retry_with_auto_tool_choice(params)
                    and _is_forced_tool_choice_unsupported_error(exc)
                ):
                    logger.warning(
                        "Forced tool_choice rejected by upstream; retrying with tool_choice='auto' "
                        "for model=%s",
                        params.get("model"),
                    )
                    params = dict(params)
